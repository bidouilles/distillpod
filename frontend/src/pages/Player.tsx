...
export default function Player() {
  ...
  const {
    ...
    audioReady, loadEpisode, preparing, preparingNote,
  ...
  return (
    <div className="pb-2">
      ...
      {/* What the wait is for. The provider has always known — nothing rendered
          it, so a download behind the nightly job's turn looked identical to a
          broken button. */}
      {preparing === episodeId && (
        <div className="flex items-center gap-2 text-xs text-gray-400 bg-gray-900 rounded-xl px-4 py-2.5 mb-4">
          <span className="w-3 h-3 border-2 border-gray-600 border-t-indigo-400 rounded-full animate-spin inline-block flex-shrink-0" />
          <span className="leading-relaxed">{preparingNote || "Fetching the audio…"}</span>
        </div>
      )}
