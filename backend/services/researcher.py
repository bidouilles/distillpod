...
async def library_echoes(episode_id: str, claim: str, premise: str) -> list[dict]:
    """Other moments in the library touching the same subject.

    The one thing a web search cannot answer, and it costs nothing: the same
    retrieval Ask uses, run over the claim.
    """
    from database import get_db
    from services import librarian

    query = claim or premise[:200]
    db = await get_db()
    try:
        passages = await librarian.gather(db, [query[:120]], question=query)
    except Exception as exc:
...
async def run_research(research_id: str, gist_id: str) -> dict:
    ...
    echoes = await library_echoes(gist["episode_id"], claim, premise)
    ...
    _update(research_id, "done", file_path=str(file_path), public_url=public_url,
            finished_at=datetime.now(timezone.utc).isoformat())
